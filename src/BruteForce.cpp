...
uint32_t BruteForce::Knapsack(uint8_t n, int32_t M, Item *items, TaskType type, int32_t B){
    ...
    // (2) not included
        ...
        return std::max( 
                        (uint32_t) (items[n - 1].value + BruteForce::Knapsack(n-1, M - items[n - 1].weight, items, type, B - items[n - 1].value)),
